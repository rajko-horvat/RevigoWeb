...
namespace RevigoWeb
{
	public partial class GetRevigoStats : System.Web.UI.Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				// fix aggregated data for report statistics
				/*DateTime dtFrom1 = new DateTime(2022, 12, 13);
				DateTime dtTo1 = dtFrom1.AddHours(1.0);

				MySqlCommand oCommand1 = new MySqlCommand(
					string.Format("delete from {0} where ({0}.DateTimeTicks>=?from);", "stats_h"),
					oConnection.Connection);
				oCommand1.Parameters.Add("?from", MySqlDbType.Int64).Value = dtFrom1.Ticks;
				...
				while (true)
				{
					...
					MySqlDataAdapter oAdapter1 = new MySqlDataAdapter(string.Format("select RequestSource, Cutoff, ValueType, SpeciesTaxon, Measure, RemoveObsolete, " +
						"BiologicalProcess, CellularComponent, MolecularFunction, ExecTicks, Count, NSCount, " +
						"Table_BP, Table_CC, Table_MF, Scatterplot_BP, Scatterplot_CC, Scatterplot_MF, Scatterplot3D_BP, Scatterplot3D_CC, Scatterplot3D_MF, " +
						"Cytoscape_BP, Cytoscape_CC, Cytoscape_MF, TreeMap_BP, TreeMap_CC, TreeMap_MF, TagClouds, " +
						"SimMat_BP, SimMat_CC, SimMat_MF from {0} where ({0}.DateTimeTicks>=?from) and ({0}.DateTimeTicks<?to) order by {0}.ID;", "stats"),
						oConnection.Connection);
					oAdapter1.SelectCommand.Parameters.Add("?from", MySqlDbType.Int64).Value = dtFrom1.Ticks;
					oAdapter1.SelectCommand.Parameters.Add("?to", MySqlDbType.Int64).Value = dtTo1.Ticks;
					oAdapter1.Fill(dtResult1);
					oAdapter1.Dispose();
					...
					if (dtResult1.Rows.Count > 0)
					{
						...
						{
							DataRow row = dtResult1.Rows[i];

							// aggregate statistics by hours
							oCommand1 = new MySqlCommand("select ID " +
								"from stats_h " +
								...
								oConnection.Connection);
							oCommand1.Parameters.Add("?vDateTimeTicks", MySqlDbType.Int64).Value = dtFrom1.Ticks;
							oCommand1.Parameters.Add("?vRequestSource", MySqlDbType.Int32).Value = row[0];
							oCommand1.Parameters.Add("?vCutoff", MySqlDbType.Int32).Value = row[1];
							oCommand1.Parameters.Add("?vValueType", MySqlDbType.Int32).Value = row[2];
							oCommand1.Parameters.Add("?vSpeciesTaxon", MySqlDbType.Int32).Value = row[3];
							oCommand1.Parameters.Add("?vMeasure", MySqlDbType.Int32).Value = row[4];
							oCommand1.Parameters.Add("?vRemoveObsolete", MySqlDbType.Int32).Value = row[5];
							oCommand1.Parameters.Add("?vNSCount", MySqlDbType.Double).Value = row[11];
							//oCommand.CommandTimeout = 240;
							object id = oCommand1.ExecuteScalar();
							...
							if (id != null && id != DBNull.Value)
							{
								...
								oCommand1 = new MySqlCommand(
									...
									"Table_BP=Table_BP+?vTable_BP, Table_CC=Table_CC+?vTable_CC, Table_MF=Table_MF+?vTable_MF, "+
									"Scatterplot_BP=Scatterplot_BP+?vScatterplot_BP, Scatterplot_CC=Scatterplot_CC+?vScatterplot_CC, Scatterplot_MF=Scatterplot_MF+?vScatterplot_MF, "+
									"Scatterplot3D_BP=Scatterplot3D_BP+?vScatterplot3D_BP, Scatterplot3D_CC=Scatterplot3D_CC+?vScatterplot3D_CC, Scatterplot3D_MF=Scatterplot3D_MF+?vScatterplot3D_MF, "+
									"Cytoscape_BP=Cytoscape_BP+?vCytoscape_BP, Cytoscape_CC=Cytoscape_CC+?vCytoscape_CC, Cytoscape_MF=Cytoscape_MF+?vCytoscape_MF, "+
									"TreeMap_BP=TreeMap_BP+?vTreeMap_BP, TreeMap_CC=TreeMap_CC+?vTreeMap_CC, TreeMap_MF=TreeMap_MF+?vTreeMap_MF, "+
									"TagClouds=TagClouds+?vTagClouds, "+
									"SimMat_BP=SimMat_BP+?vSimMat_BP, SimMat_CC=SimMat_CC+?vSimMat_CC, SimMat_MF=SimMat_MF+?vSimMat_MF " +
									"where ID=?id;", oConnection.Connection);
								oCommand1.Parameters.Add("?vBiologicalProcess", MySqlDbType.Int64).Value = row[6];
								oCommand1.Parameters.Add("?vCellularComponent", MySqlDbType.Int64).Value = row[7];
								oCommand1.Parameters.Add("?vMolecularFunction", MySqlDbType.Int64).Value = row[8];
								oCommand1.Parameters.Add("?vExecTicks", MySqlDbType.Int64).Value = row[9];
								oCommand1.Parameters.Add("?vTable_BP", MySqlDbType.Double).Value = row[12];
								oCommand1.Parameters.Add("?vTable_CC", MySqlDbType.Double).Value = row[13];
								oCommand1.Parameters.Add("?vTable_MF", MySqlDbType.Double).Value = row[14];
								oCommand1.Parameters.Add("?vScatterplot_BP", MySqlDbType.Double).Value = row[15];
								oCommand1.Parameters.Add("?vScatterplot_CC", MySqlDbType.Double).Value = row[16];
								oCommand1.Parameters.Add("?vScatterplot_MF", MySqlDbType.Double).Value = row[17];
								oCommand1.Parameters.Add("?vScatterplot3D_BP", MySqlDbType.Double).Value = row[18];
								oCommand1.Parameters.Add("?vScatterplot3D_CC", MySqlDbType.Double).Value = row[19];
								oCommand1.Parameters.Add("?vScatterplot3D_MF", MySqlDbType.Double).Value = row[20];
								oCommand1.Parameters.Add("?vCytoscape_BP", MySqlDbType.Double).Value = row[21];
								oCommand1.Parameters.Add("?vCytoscape_CC", MySqlDbType.Double).Value = row[22];
								oCommand1.Parameters.Add("?vCytoscape_MF", MySqlDbType.Double).Value = row[23];
								oCommand1.Parameters.Add("?vTreeMap_BP", MySqlDbType.Double).Value = row[24];
								oCommand1.Parameters.Add("?vTreeMap_CC", MySqlDbType.Double).Value = row[25];
								oCommand1.Parameters.Add("?vTreeMap_MF", MySqlDbType.Double).Value = row[26];
								oCommand1.Parameters.Add("?vTagClouds", MySqlDbType.Double).Value = row[27];
								oCommand1.Parameters.Add("?vSimMat_BP", MySqlDbType.Double).Value = row[28];
								oCommand1.Parameters.Add("?vSimMat_CC", MySqlDbType.Double).Value = row[29];
								oCommand1.Parameters.Add("?vSimMat_MF", MySqlDbType.Double).Value = row[30];
								oCommand1.Parameters.Add("?id", MySqlDbType.Int64).Value = Convert.ToInt64(id);
								oCommand1.ExecuteNonQuery();
							}
							else
							{
								...
								oCommand1 = new MySqlCommand(
									...
									"Table_BP, Table_CC, Table_MF, Scatterplot_BP, Scatterplot_CC, Scatterplot_MF, Scatterplot3D_BP, Scatterplot3D_CC, Scatterplot3D_MF, " +
									"Cytoscape_BP, Cytoscape_CC, Cytoscape_MF, TreeMap_BP, TreeMap_CC, TreeMap_MF, TagClouds, " +
									"SimMat_BP, SimMat_CC, SimMat_MF) values " +
									...
									"?vTable_BP, ?vTable_CC, ?vTable_MF, ?vScatterplot_BP, ?vScatterplot_CC, ?vScatterplot_MF, ?vScatterplot3D_BP, ?vScatterplot3D_CC, ?vScatterplot3D_MF, " +
									"?vCytoscape_BP, ?vCytoscape_CC, ?vCytoscape_MF, ?vTreeMap_BP, ?vTreeMap_CC, ?vTreeMap_MF, ?vTagClouds, " +
									"?vSimMat_BP, ?vSimMat_CC, ?vSimMat_MF);",
									oConnection.Connection);
								oCommand1.Parameters.Add("?vDateTimeTicks", MySqlDbType.Int64).Value = dtFrom1.Ticks;
								oCommand1.Parameters.Add("?vRequestSource", MySqlDbType.Int32).Value = row[0];
								oCommand1.Parameters.Add("?vCutoff", MySqlDbType.Int32).Value = row[1];
								...
								oCommand1.Parameters.Add("?vMolecularFunction", MySqlDbType.Int64).Value = row[8];
								oCommand1.Parameters.Add("?vExecTicks", MySqlDbType.Int64).Value = row[9];
								oCommand1.Parameters.Add("?vCount", MySqlDbType.Int64).Value = row[10];
								oCommand1.Parameters.Add("?vNSCount", MySqlDbType.Double).Value = row[11];
								oCommand1.Parameters.Add("?vTable_BP", MySqlDbType.Double).Value = row[12];
								oCommand1.Parameters.Add("?vTable_CC", MySqlDbType.Double).Value = row[13];
								oCommand1.Parameters.Add("?vTable_MF", MySqlDbType.Double).Value = row[14];
					...
					{
						if (dtTo1 > DateTime.Now)
							break;
					}

					dtFrom1 = dtTo1;
					dtTo1 = dtFrom1.AddHours(1.0);
				}*/

				// parameters
				DateTime dtFrom;
				DateTime dtTo;
				string sFrom = Convert.ToString(Request.Params["from"]);
				string sTo = Convert.ToString(Request.Params["to"]);
					...
					DateTime.TryParseExact(sTo, "d.M.yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out dtTo))
				{
					...
					try
					{
						...
						if (dtResult.Rows.Count > 0)
						{
							...
							for (int i = 0; i < aSpeciesAnnotations.Items.Count; i++)
							{
								aColumns.Add(string.Format("{0} [{1}]",
									Global.StringToJSON(aSpeciesAnnotations.Items[i].SpeciesName), aSpeciesAnnotations.Items[i].TaxonID));
							}
							aViewColumns.Add(aColumns);
							...
							while (iPosition < dtResult.Rows.Count)
							{
								...
								while (iPosition < dtResult.Rows.Count && (lTicks = DBConnection.ToInt64((row = dtResult.Rows[iPosition])[0])) < lNewFromTicks)
								{
									for (int j = 0; j < aValues.Length; j++)
									{
										aValues[j] += DBConnection.ToDouble(row[j + 1]);
									}
									iPosition++;
								}
							...
							writer.Write("{{\"error\":\"\",\"maxDate\":\"{0}\",\"requestCount\":{1},\"views\":[", 
							...
							for (int i = 0; i < aViews.Count; i++)
							{
								...
								for (int j = 0; j < aViewColumns[i].Count; j++)
								{
									if (j > 0)
										sbColumns.Append(",");
									sbColumns.AppendFormat("\"{0}\"", Global.StringToJSON(aViewColumns[i][j]));
								...
								writer.Write("{{\"name\":\"{0}\",\"type\":\"{1}\",\"columns\":[{2}],\"distribution\":[{3}],\"data\":[{4}]}}",
						...
						{
							writer.WriteLine("{{\"error\":\"No data points are available for specified date range\",\"maxDate\":\"{0}\",\"requestCount\":0,\"views\":[]}}",
								(new DateTime(Global.MaxStatTicks)).ToString("d.M.yyyy"));
						}
					}
					catch (Exception ex)
					{
						writer.WriteLine("{{\"error\":\"Error accessing data\",\"maxDate\":\"{0}\",\"requestCount\":0,\"views\":[]}}",
							(new DateTime(Global.MaxStatTicks)).ToString("d.M.yyyy"));
				...
				{
					writer.WriteLine("{{\"error\":\"Invalid date range specified\",\"maxDate\":\"{0}\",\"requestCount\":0,\"views\":[]}}",
						(new DateTime(Global.MaxStatTicks)).ToString("d.M.yyyy"));
				}
				oConnection.Close();
				writer.Flush();
			...
			writer.WriteLine("{{\"error\":\"No database available\",\"maxDate\":{0},\"requestCount\":0,\"views\":[]}}",
				(new DateTime(Global.MaxStatTicks)).ToString("d.M.yyyy"));
			writer.Flush();
			Response.End();
		}
